...
for(const scenario of ['archive','active','unrelated','staged','staged-all','unpublished','remote-conflict','existing']){
 ...
 if(scenario==='staged-all')run(repo,'add','.');
 ...
 if(['archive','staged','staged-all'].includes(scenario)){assert.equal(result.status,0,result.stderr);assert.equal(run(repo,'status','--porcelain'),'');const dir=fs.readdirSync(path.join(repo,'.git/daily-recovery-archives'))[0];assert.equal(fs.readFileSync(path.join(repo,'.git/daily-recovery-archives',dir,'draft/news/data/2026-09-17.json'),'utf8'),'draft article\n');assert(!fs.existsSync(path.join(repo,'.git/daily-publish.lock')));}else{assert.notEqual(result.status,0);assert(fs.existsSync(path.join(repo,'.git/daily-publish.lock/state.json')));assert.equal(fs.readFileSync(path.join(repo,'news/data/2026-09-17.json'),'utf8'),'draft article\n');}
