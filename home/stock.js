...
(() => {
  ...
  function data(){if(!demo)return state?.markets?.KR||{};return {...demoData(),net_profit:9800,equity:1009800,cash:1009800,total:9800,today:-400,fees:120,initial_equity:1000000,equity_series:[0,1200,800,2400,1800,3100,4600,3900,5200,4900,6300,6100,7200,6800,8100,7800,9300,9000,10200,9800]};}
  function render(){
    ...
    $('chart-label').textContent=chartKind==='price'?(d.label?d.label+(demo?' | 5분봉 · 09:00–13:00 KST · KRW':' | 가격 자료 미연결'):'종목·기간·통화 확인 대기'):'누적 총손익 · '+(demo?'가상 계좌 · 20개 시점 예시':'실현·평가손익 포함 · 입출금 제외');
    ...
    $('chart-unit').textContent=chartKind==='price'?(candle?'라인으로 전환':'캔들로 전환'):(percent?'금액으로 전환':'%로 전환');
  ...
  $('chart-unit').onclick=()=>{if(chartKind==='price'){candle=!candle;}else percent=!percent;render();};
