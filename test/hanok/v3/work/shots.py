"""헤드리스 크롬으로 화면 4개 × 폭 4개 스크린샷을 찍고, 통합 전후를 픽셀 단위로 비교합니다.
  ...
  python work/shots.py shoot <라벨> <앱 URL 경로>      예: python work/shots.py shoot before /v3/index.html
...
def shoot(label, src):
    ...
    for w, h, st in jobs:
        if True:
            ...
            ww = max(w, 500)
            url = f"{BASE}/v3/work/shot.html?src={src}&state={st.replace('-tall', '')}&w={w}&h={h}"
...
def combos(name, src):
    src = norm(src)
    url = f"{BASE}/v3/work/shot.html?src={src}&mode=combos&name={name}"
