/* ---------- 홈페이지 관리 (더보기 → 홈페이지 관리, 관리자 비밀번호) ----------
   가게 사이트(../)의 글·사진·팝업·차림·영업시간·연락처와 '홈페이지 예약' 규칙을 여기서 고칩니다. 설정(store().settings)과는 별개입니다.
...
function saI(path, label, note){   /* 사진: 파일명/주소 + 작은 보기 (3단계에서 '올리기' 붙음) */
  var v = saGet(path); if(v == null) v = "";
...
function saTabNotices(){
  ...
  return '<p class="f-note" style="margin:0 0 12px">홈페이지 첫 화면에 겹쳐 뜨는 알림창입니다. 마감일이 지나면 저절로 안 뜹니다. 손님이 닫으면 그 세션 동안, "오늘 하루 보지 않기" 면 그날 안 뜹니다.</p>' +
    list.map(function(n, i){
      var p = "notices." + i + ".", off = n.until && n.until < todayStr();
    ...
    '<div class="btn-row"><button class="btn" onclick="saArrAdd(\'notices\', {id:\'n\' + Date.now().toString(36), until:\'\', x:40 + 30 * ' + list.length + ', y:110 + 30 * ' + list.length + ', w:380, title:\'\', lines:[], button:\'예약하기\'})">팝업 추가</button></div>';
}
/* 3. 영업시간·연락처 */
function saTabHours(){
  ...
  return saBox("영업시간 (홈페이지 표시용)",
    ...
    saBox("링크",
      ...
      saF("info.menuPdf", "메뉴판 PDF", "파일명(menu.pdf) 또는 올린 파일 주소")) +
...
function saTabImages(){
  ...
  return '<p class="f-note" style="margin:0 0 12px">지금은 <b>img/ 폴더의 파일명</b> 또는 <b>전체 주소</b>를 적습니다. 태블릿·폰에서 사진을 바로 올리는 것(3단계)은 다음 작업입니다.</p>' +
