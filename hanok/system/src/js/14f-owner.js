...
async function openCloseDay(){ view.form = {type:"closeday", page:true}; render(); }
function sheetCloseDay(){
  var today = todayStr(), nowM = toMin(nowHM());
  var left = store().reservations.filter(function(r){ return r.date === today && r.status === "확정"; }).sort(function(a,b){ return a.time.localeCompare(b.time); });
  var rows = left.map(function(r){ return '<div class="rowitem"><span class="time-col">' + esc(hm(r.time)) + '</span><span class="grow"><span class="t">' + esc(r.name) + tierTag(r) + groupTag(r) + '</span><span class="s">' + esc(pplText(r)) + ' · ' + esc(resSeatLabel(r)) + '</span></span>' +
    '<span class="btn-row"><button class="btn sm primary" onclick="cdMark(\'' + r.id + '\', \'방문\')">방문</button><button class="btn sm ghost danger" onclick="cdMark(\'' + r.id + '\', \'노쇼\')">노쇼</button></span></div>'; }).join("");
  var done = left.length === 0;
  return '<div class="cd-wrap">' +
    '<div class="cd-hello"><b>오늘도 고생 많으셨습니다.</b><small>' + esc(dateLabel(today)) + ' · 문 닫기 전에 세 가지만 정리하면 내일 아침이 편합니다.</small></div>' +
    '<div class="cd-step ' + (done ? "done" : "") + '"><div class="cd-n">1</div><div class="cd-b"><b>오늘 손님 방문 처리</b>' +
      (done ? '<small>오늘 예약이 모두 정리됐습니다.</small>' : '<small>아직 \'확정\' 으로 남은 ' + left.length + '건 — 온 손님은 방문, 안 온 손님은 노쇼. 감사 문자는 방문으로 표시한 손님께만 갑니다.</small><div class="card searchbox" style="margin-top:8px">' + rows + '</div>') + '</div></div>' +
