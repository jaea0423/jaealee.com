...
async function openCloseDay(date){ view.form = {type:"closeday", page:true, date:date || todayStr()}; render(); }
function sheetCloseDay(){
  var today = (view.form && view.form.date) || todayStr(), nowM = toMin(nowHM());
  ...
  return '<div class="cd-wrap">' +
    ...
    '<div class="cd-step ' + (done ? "done" : "") + '"><div class="cd-n">1</div><div class="cd-b"><b>오늘 손님 방문 처리</b>' +
      (done ? '<small>예약이 모두 정리됐습니다.</small>' : '<small>아직 \'확정\' 으로 남은 ' + left.length + '건 — 온 손님은 방문, 안 온 손님은 노쇼. 감사 문자는 방문으로 표시한 손님께만 갑니다.</small><div class="card searchbox" style="margin-top:8px">' + rows + '</div>') + '</div></div>' +
