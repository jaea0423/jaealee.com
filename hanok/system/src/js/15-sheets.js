...
async function adminGate(what){
  ...
  /* 아직 6자리로 안 바꾼 서버는 옛 비밀번호(영문 섞임·긴 것)를 쳐야 하므로 글자 수·숫자 제한을 두지 않습니다 */
  const pw = await uiPrompt(what, "사장님 2차 비밀번호(숫자 6자리 · 아직 안 정했으면 예전 관리자 비밀번호)를 입력하세요", {password:true, ok:"확인"});
