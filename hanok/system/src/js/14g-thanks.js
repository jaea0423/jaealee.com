/* ---------- 감사 문자 (thanks_sms, 18차 재아) ----------
   ...
   Gemini 키는 설정(stores.settings.ai) — 직원 로그인이면 볼 수 있으니 무료 키를 쓰고, 유출되면 구글 콘솔에서 바꾸세요. */
var TH = null;
var TH_DEF_MODEL = "gemini-2.5-flash";
function thCfg(){ var a = store().settings.ai || {}; return { key: a.geminiKey || "", model: a.model || TH_DEF_MODEL, hour: a.thanksHour || "11:00", common: a.thanksCommon || "" }; }
...
async function thGemini(prompt){
  ...
  var res;
  try{
    res = await fetch("https://generativelanguage.googleapis.com/v1beta/models/" + encodeURIComponent(cfg.model) + ":generateContent?key=" + encodeURIComponent(cfg.key), {
      method:"POST", headers:{ "Content-Type":"application/json" }, signal:ctl.signal,
      body: JSON.stringify({ contents:[{ role:"user", parts:[{ text:prompt }] }], generationConfig:{ temperature:0.9, maxOutputTokens:400 } }) });
  }finally{ clearTimeout(t); }
  if(!res.ok){ var m = ""; try{ m = (await res.json()).error.message || ""; }catch(e){} throw new Error("HTTP " + res.status + (m ? " · " + m : "")); }
  var j = await res.json(), text = "";
  try{ text = j.candidates[0].content.parts.map(function(p){ return p.text || ""; }).join("").trim(); }catch(e){}
...
async function thMake(resId, force){
  ...
  try{
    ...
    var why = e.name === "AbortError" ? "응답이 20초 안에 안 옴" : e.message === "NOKEY" ? "Gemini 키가 없음(설정)" : e.message;
...
function thSetupHtml(){
  ...
  return '<div class="overlay" onclick="TH.setup=false; render()"><div class="sheet" onclick="event.stopPropagation()">' + sheetHead("AI 설정 (Gemini)") +
    '<label class="f"><div class="lb">Gemini API 키</div><input id="th-key" type="password" value="' + esc(cfg.key) + '" placeholder="AIza…" autocomplete="off"></label>' +
    '<p class="f-note">Google AI Studio(aistudio.google.com) → Get API key 에서 무료 키를 만듭니다. 이 키는 매장 설정에 저장돼 직원 로그인에서도 보일 수 있으니 무료 키를 쓰고, 유출되면 콘솔에서 바꾸세요.</p>' +
    '<div class="grid2"><label class="f"><div class="lb">모델</div><input id="th-model" type="text" value="' + esc(cfg.model) + '"></label><label class="f"><div class="lb">보내는 시각 기본값</div><input id="th-hour" type="time" value="' + esc(cfg.hour) + '"></label></div>' +
    '<div class="sheet-actions"><button class="btn ghost" onclick="TH.setup=false; render()">취소</button><button class="btn primary" onclick="thSaveCfg({geminiKey:document.getElementById(\'th-key\').value.trim(), model:document.getElementById(\'th-model\').value.trim()||TH_DEF_MODEL, thanksHour:document.getElementById(\'th-hour\').value||\'11:00\'}); TH.setup=false; render()">저장</button></div></div></div>';
}
