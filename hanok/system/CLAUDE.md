# system/ — 한옥반점 예약 관리 시스템 (옛 이름 v7)

**여기가 시스템 작업 공간입니다.** 사이트는 한 칸 위 `../`(`hanok/CLAUDE.md` 부터 읽으세요).
v6 까지의 옛 버전은 `../../test/hanok/` 에 보관(읽기만). v7 은 v6 복사본에서 **보안·오류 전수조사, 불필요한 코드 제거, 파일 나누기**를 한 것이고, 2026-09-16 에 `hanok/system/` 으로 옮겨 최종본 자리가 됐습니다.
...
| 경로 | 상태 |
|---|---|
| `../../test/hanok/` (v1~v6·옛 문서) | **읽기만** |
| `assets/*.b64` | 이미지·글꼴 base64. 빌드가 여기서 가져옵니다(옛 test/hanok/assets · v4/assets 에서 복사해 옴) |
| `src/page.html` | HTML 뼈대. `__CSS__` `__JS__` 자리에 아래 조각이 들어갑니다 |
| `src/css/NN-이름.css` | CSS 조각 — **이름 순서대로** 이어 붙습니다(순서가 곧 우선순위) |
| `src/js/NN-이름.js` | JS 조각 — 이름 순서대로 이어 붙습니다. 전부 한 `<script>` 안이라 서로 전역으로 봅니다 |
| `build.py` | 조각을 합쳐 HTML 하나로. `__SUPA_CFG__` 에 설정 파일을 끼웁니다 |
| `work/patchlib.py` | 패치 스크립트 공용(`load/save/rep/where/js_check`) |
| `work/index.v6.html` | 나누기 전 원본(참고용, 고치지 않음) |
| `supabase/` `backup/` | v6 그대로 |
...
배포 주소: **`jaealee.com/hanok/system`** (개발용은 `…/system/dev/`). 배포 형태는 여전히 **HTML 파일 하나**입니다.
...
2. 네 폭(430/820/1280/1920) + 손님 화면 확인. 스크린샷 `python work/shot.py`, 동작은 브라우저에서 JS 로(dev 사이트 `http://127.0.0.1:8767/system/dev/` — 8767 은 `hanok/` 루트)
