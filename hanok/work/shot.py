...
for p in pages:
    ...
    url = "http://127.0.0.1:8767/%s.html%s&v=%d" % ("index" if p == "notice" else p, q, time.time())
...
if "rv" in (sys.argv[4] if len(sys.argv) > 4 else ""):
    for n in (1,2,3,4,5,6,7):
        ...
        subprocess.run([CHROME, "--headless=new", "--disable-gpu", "--hide-scrollbars", "--no-first-run",
                        ...
                        "--virtual-time-budget=4000", "--screenshot=" + png, "http://127.0.0.1:8767/reserve.html?shot=1&rv=%d&v=%d" % (n, time.time())], capture_output=True, timeout=120)
        print(png)
